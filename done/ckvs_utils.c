...
int SHA256_from_string(const char *in, struct ckvs_sha *sha){
    int ret = hex_decode(in, sha->sha);
    return ret;
}

//divises la taille par 2
int hex_decode(const char *in, uint8_t *buf){
    //iterer sur les elements de in 2 par 2
    //appliquer strtoul sur 2 elements a la fois
    if(in == NULL || buf == NULL){
        return -1;
    }
