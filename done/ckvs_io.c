...
int ckvs_find_entry(struct CKVS *ckvs, const char *key, const struct ckvs_sha *auth_key, struct ckvs_entry **e_out){
    ...
    *e_out = &ckvs->entries[idx];
    ...
    int ret = ckvs_cmp_sha(auth_key,&ckvs->entries[idx].auth_key);
    if(ret != 0) {
        debug_printf("auth_key = %.32s", auth_key->sha);
        debug_printf("entry auth_key = %.32s", ckvs->entries[idx].auth_key.sha);
        debug_printf("size of the thing = %d", strlen(ckvs->entries[idx].auth_key.sha));
        M_EXIT(ERR_DUPLICATE_ID, "the password couldn't be validated (ckvs_find_entry), ckvs_cmp_sha = %d", ret);
    }
    return ERR_NONE;
}
