 ...
 * @return int (error_code) : ERR_NONE if the HMAC was computed correctly, ERR_INVALID_COMMAND if the computation
...
int ckvs_compute_HMAC(const void *key,const unsigned char *d,size_t len_d,unsigned char *md){
    ...
    if(sizeOutput != SHA256_DIGEST_LENGTH){
        M_EXIT(ERR_INVALID_COMMAND,
               "HMAC of %s doesn't have correct value : "
               "(obtained) %d != %d (wanted) (ckvs_crypto)",d, sizeOutput, SHA_DIGEST_LENGTH);
    }
    return ERR_NONE;
}
...
int ckvs_client_encrypt_pwd(ckvs_memrecord_t *mr, const char *key, const char *pwd) {
    ...
    char stretched[2*CKVS_MAXKEYLEN+2] = {"\0"};
    ...
    if(ptr == NULL){
        M_EXIT(ERR_INVALID_COMMAND,"ERROR when computing SHA256, ckvs_encrypt_pwd");
    }
